...
export default function LoginForm() {
	...
	const onSubmits = async (data: TloginFormData) => {
		await axios
			.post(`${process.env.NEXT_PUBLIC_LARAVEL_BACKEND_API_URL}/login`, data)
			.then((response) => {
							...
							<div className="flex flex-col gap-5">
								<div className="flex flex-col gap-2">
									<input
										...
										{...register("email")}
										placeholder="Email"
										className={`bg-[#3A364D] text-white placeholder:text-[#6D6980] rounded-lg p-4 focus:border-[#3920ba] focus:border-[1px] focus:outline-none focus:ring-1 outline-none ${
